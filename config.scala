 ...
 */
class WithNLargeBooms(n: Int = 1, overrideIdOffset: Option[Int] = None) extends Config( //This is what we are using
  new WithTAGELBPD ++ // Default to TAGE-L BPD
  new Config((site, here, up) => {
    case TilesLocated(InSubsystem) => {
      val prev = up(TilesLocated(InSubsystem), site)
      ...
      (0 until n).map { i =>
        BoomTileAttachParams(
          tileParams = BoomTileParams(
            core = BoomCoreParams(
              fetchWidth = 8,  // 8-wide fetch
              decodeWidth = 3, // 3-wide decode
              numRobEntries = 96, // 96 Reorder Buffer Entries
              issueParams = Seq(
              ...
              numIntPhysRegisters = 100,  // 100 Integer Physical Registers
              numFpPhysRegisters = 96,   // 96 Floating Point Physical Registers
              numLdqEntries = 24, // 24 Load Queue Entries
              numStqEntries = 24, // 24 Store Queue Entries
              maxBrCount = 16, // 16 Branches in the TLB
              numFetchBufferEntries = 24, // 24 Fetch Buffer Entries
              ftq = FtqParameters(nEntries=32),
              nPerfCounters = 6, // 6 Performance Counters
              fpu = Some(freechips.rocketchip.tile.FPUParams(sfmaLatency=4, dfmaLatency=4, divSqrt=true))
            ),
            dcache = Some(
